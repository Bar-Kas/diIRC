...
export const InviteModal = () => {
  ...
  const updateInviteCode = useMockStore((state) => state.updateInviteCode);
  ...
  const inviteUrl = `${origin || window.location.origin}/invite/${server?.inviteCode}`;
  ...
  const onNew = async () => {
    try {
      setIsLoading(true);
      if (server?.id) {
        const newCode = updateInviteCode(server.id);
        onOpen("invite", { server: { ...server, inviteCode: newCode } });
      }
    } catch (error) {
      console.log(error);
    } finally {
